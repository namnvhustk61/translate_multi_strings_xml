...
OUTPUTlangs = ["ar","fr","fi","fa","el","hi","bn","de","it","ja","ko","in","ms","es","pt","ru","th","tr","vi","zh-CN"]
...
def serialize_text(text, language):
    ...
    text = re.sub(tag_match_regex, '',
                  text)  # Remove all html tags from text (although there should be none, but still ensuring)
                ...
                if (root[i][j].tag == 'item') & (isTranslatable != 'false'):
                    ...
                    if len(root[i][j]) != 0:
                        for element in range(len(root[i][j])):
                            root[i][j][element].text = " " + await translate(root[i][j][element].text, OUTPUTLANGUAGE,
                                                                             INPUTLANGUAGE)
                            root[i][j][element].tail = " " + await translate(root[i][j][element].tail, OUTPUTLANGUAGE,
                                                                             INPUTLANGUAGE)
    # write new xml file
